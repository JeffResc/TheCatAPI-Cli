...
if [[ " ${PATHS[*]} " == *" /usr/bin "* ]]; then
	wget -O /usr/bin/catpic -q https://raw.githubusercontent.com/JeffResc/TheCatAPI-Cli/master/catpic.sh
	chmod +x /usr/bin/catpic
	echo "Successfully installed catpic to /usr/bin"
elif [[ " ${PATHS[*]} " == *" /bin "* ]]; then
	wget -O /bin/catpic -q https://raw.githubusercontent.com/JeffResc/TheCatAPI-Cli/master/catpic.sh
	chmod +x /bin/catpic
	echo "Successfully installed catpic to /bin"
elif [[ " ${PATHS[*]} " == *" /usr/sbin "* ]]; then
	wget -O /usr/sbin/catpic -q "https://raw.githubusercontent.com/JeffResc/TheCatAPI-Cli/master/catpic.sh"
